...
function updateGame(pongGame, info) {
	...
	// 패들 위치 결정
	const myPaddle = paddles.player1.user_id === userId ? paddles.player1 : paddles.player2;
	const opponentPaddle = paddles.player1.user_id === userId ? paddles.player2 : paddles.player1;
...
function handleGameOver(pongGame, data, socket) {
    ...
    setTimeout(() => changeUrl(pongGame, data, "/endgame"), 0);
}
...
export default function RemoteBoard(info, rightUser_id) {
    ...
    const opponentInfo = info.player_info.find(player => player.user_id !== rightUser_id);

	canvas.height = 550;
	canvas.width = canvas.height * 1.45;
    const pongGame = createPongGame(canvas, info.option);
